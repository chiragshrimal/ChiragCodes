...
bool cycle_detected(int start , unordered_map<int,vector<int>>&map,vector<int>&visited,vector<int>&path){
    visited[start]=1;
    path[start]=1;
...
int main(){
    ...
    vector<int>path(n,0);
    for(int i=0;i<n;i++){
        if(visited[i]==0 && cycle_detected(i,map,visited,path)==true){
            cout<<"yes"<<endl;
            return true;
        }
    }
