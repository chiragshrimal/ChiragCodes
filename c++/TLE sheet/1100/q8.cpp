...
void solve(){
    ...
    ll ans=abs(arr[a][0]-arr[b][0])+abs(arr[a][1]-arr[b][1]);
    ll d1=1e18;
    ll d2=1e18;
    for(int i=0;i<k;i++){
        d1=min(d1,abs(arr[a][0]-arr[i][0])+abs(arr[a][1]-arr[i][1]));
        d2=min(d2,abs(arr[b][0]-arr[i][0])+abs(arr[b][1]-arr[i][1]));
    }
    cout<<min(ans,d1+d2)<<endl;
    return ;
}
